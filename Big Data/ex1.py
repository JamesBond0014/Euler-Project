...
class NotEnoughFuel(Exception):
    ...
    def _init_car(self,fuel):
        self.fuel=fuel
        self.x=0
        self.y=0
    def _delta_movement(self, x2, y2):
        """Returns unit distance moved"""
        return abs(x2-self.x)+abs(y2-self.y)
    def move(self,x2,y2):
        """Moves car from previous position to (x2,y2)
        Then reduces the car's fuel based on how much it moved.

        If not enough fuel is available, the car's fuel and position do not change.
        Instead, prompted with NotEnoughFuel
        """

        delta = self._delta_movement(x2,y2)
        if delta > self.fuel():
            raise NotEnoughFuel

    fuel_remaining=self.fuel-self._delta_movement
